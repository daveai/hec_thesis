...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[scale=0.9]
        ...
        \node[white, font=\small] at (6,8.9) {\$7.4B Outstanding | \textasciitilde\$30M Annual Servicing};
        ...
        \node[black, font=\small] at (6,7.4) {\$1.37B | \$137M Annual | Cumulative (compounds to 18\%)};

        % 3. STRC - Variable (yellow-green lighter)
        \fill[seniorpref!80!white, rounded corners=3pt] (0,5.5) rectangle (12,6.8);
        \node[black, font=\bfseries] at (6,6.4) {STRC -- Variable-Rate Credit Preferred};
        \node[black, font=\small] at (6,5.9) {\$3.38B | \textasciitilde\$372M Annual | Cumulative};

        % 4. STRE - Euro (yellow)
        \fill[midpref, rounded corners=3pt] (0,4) rectangle (12,5.3);
        \node[black, font=\bfseries] at (6,4.9) {STRE -- 10\% Euro-Denominated Preferred};
        \node[black, font=\small] at (6,4.4) {\texteuro775M (\textasciitilde\$0.8B) | \texteuro78M Annual | Cumulative};

        % 5. STRK - Strike (orange)
        \fill[juniorpref, rounded corners=3pt] (0,2.5) rectangle (12,3.8);
        \node[black, font=\bfseries] at (6,3.4) {STRK -- 8\% Strike Convertible Preferred};
        \node[black, font=\small] at (6,2.9) {\$1.54B | \$123M Annual | Convertible to MSTR};
        ...
        \node[black, font=\small] at (6,1.4) {\$1.44B | \$144M Annual | Non-Cumulative};
        ...
        \node[font=\small, align=center] at (6,-1.3) {%
            \textbf{Total Annual Servicing:} \textasciitilde\$890M \\
            (Convertible Debt + All Preferred Dividends)
        };
    \end{tikzpicture}
    \caption[Strategy Capital Structure Waterfall]{Strategy Capital Structure Waterfall. The figure shows the seniority hierarchy of MSTR's financing instruments, from most senior (convertible debt) to most junior (common equity). Annual servicing costs total approximately \$890 million.}
    \label{fig:capital_structure}
\end{figure}

\subsection{Convertible Debt}

The convertible notes sit atop the capital structure and represent the most senior claim. Strategy has \$7.4 billion in convertible notes outstanding, with maturities staggered from 2028 to 2032. Most carry zero or minimal coupons, reducing annual cash servicing to roughly \$30 million on \$7.4 billion outstanding, a cheap cost of debt. The notes convert to MSTR shares at premiums to the issuance price, and in liquidation, convertible holders would be paid before anyone else.
...
\begin{table}[H]
\centering
\caption[Strategy Preferred Share Instruments]{Summary of Strategy Preferred Share Instruments (January 2026)}
\label{tab:preferred_summary}
\begin{tabular}{@{}llrrll@{}}
\toprule
\textbf{Ticker} & \textbf{Name} & \textbf{Notional} & \textbf{Annual Cost} & \textbf{Rate} & \textbf{Cumulative?} \\
\midrule
STRF & Strife & \$1.37B & \$137M & 10\% fixed & Yes (to 18\%) \\
STRC & Stretch & \$3.38B & \textasciitilde\$372M & 11\% variable & Yes \\
STRE & Stream & \$0.80B & \textasciitilde\$80M & 10\% fixed & Yes \\
STRK & Strike & \$1.54B & \$123M & 8\% fixed & Yes \\
STRD & Stride & \$1.44B & \$144M & 10\% fixed & No \\
\midrule
\multicolumn{2}{l}{\textbf{Total Preferred}} & \textbf{\$8.53B} & \textbf{\textasciitilde\$856M} & & \\
\bottomrule
\end{tabular}
\end{table}

Note: These figures represent current outstanding amounts, which continue to grow as Strategy executes its ``42/42'' capital plan targeting \$84 billion in total issuance by 2027.
...
The different series target different appetites. STRF offers 10\% fixed with cumulative compounding to 18\% if dividends are missed (a punishing feature that incentivizes payment). STRC started at 9\% and adjusts monthly to anchor the share price near par; by January 2026 it had reached 11\%, making it the single largest servicing obligation. STRK converts to common equity at \$1,000 per MSTR share, providing upside exposure. STRE taps European investors through the Luxembourg Euro MTF exchange. STRD, the junior tranche, offers 10\% on \$1.44 billion but non-cumulative dividends: if Strategy misses a payment, STRD holders have no legal claim to catch up.

\subsection{Seniority Analysis}

In distress, claims get paid in order: convertible debt first, then STRF, then STRC, then STRE, then STRK, then STRD, and finally common equity if anything remains. This waterfall creates synthetic subordination. The preferred layers provide a cushion that improves the credit quality of senior instruments, at the cost of concentrating risk in the junior tranches.
...
Strategy maintains a dedicated USD reserve for preferred dividend payments and debt service. As of January 2026, this reserve stood at \$2.25 billion, funded entirely from ATM equity proceeds, making it contingent on the same capital market access the broader strategy requires. At current servicing costs of approximately \$890 million annually, the reserve covers roughly 2.5 years of obligations, but the legacy analytics business generated minimal and recently slightly negative operating cash flow, so the reserve is the primary liquidity buffer. Each new preferred issuance increases the servicing burden proportionally. The contingency is real but tied to the reflexive loop: in a drawdown scenario where equity issuance shuts down, the reserve cannot be replenished.

\subsection{Implications}

The capital structure reveals several dynamics relevant to the rest of this analysis. Strategy has constructed a synthetic credit hierarchy where every tranche bears the same underlying Bitcoin risk; seniority determines loss allocation, not loss avoidance. The \textasciitilde\$890 million annual servicing cost creates a breakeven requirement that constrains the scenario analysis in Section~\ref{sec:results}. Common equity holders are effectively long a leveraged call option on Bitcoin, with senior claims acting as the strike. And the Merton model in Section~\ref{sec:methodology} can formalize this option-like payoff structure mathematically.
